...
import { createDailyTasks } from '../db/tasks';
import { DatabaseError } from '../core/errors';
import { ParsedReminder } from '../gemini';

export async function handleReminderAction(telegramId: string, reminder: ParsedReminder): Promise<string> {
    if (reminder.action === "CREATE") {
        const tasks = await createDailyTasks(telegramId, [{
            title: reminder.task,
    ...
    if (reminder.action === "RESCHEDULE" || reminder.action === "CANCEL") {
        ...
        if (error) throw new DatabaseError("Failed to find task for rescheduling", error);
        ...
        if (reminder.action === "CANCEL") {
            await supabase.from('tasks').update({ status: 'cancelled', cancelled_at: new Date().toISOString() }).eq('id', targetTask.id);
            await supabase.from('notifications').update({ status: 'CANCELLED' }).eq('task_id', targetTask.id);
