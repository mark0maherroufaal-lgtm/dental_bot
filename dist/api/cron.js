...
async function handler(req, res) {
    try {
        await (0, notifications_1.processDueNotifications)();
        ...
        const { data: upcomingEvents, error } = await supabase_1.supabase
        ...
        if (upcomingEvents && upcomingEvents.length > 0) {
            for (const event of upcomingEvents) {
                ...
                await bot_1.bot.api.sendMessage(adminId, msg, { parse_mode: "Markdown" });
                // Increment reminder count
                await supabase_1.supabase.from('academic_events').update({ reminder_sent_count: event.reminder_sent_count + 1 }).eq('id', event.id);
            }
        }
